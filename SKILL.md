...
- `v0.2.24` is source-only. Never invent or offer a native archive URL: the release has no unsigned
...
not active in `0.2.24`; a route that cannot prove an available higher tier falls back to the next
tier it can prove.
...
| Goal | Command |
|---|---|
| Install release (macOS/Linux) | `git clone …/openmayhem.git && cd openmayhem && git checkout --detach v0.2.24 && ./install.sh --from-source` |
| Install release (PowerShell) | `git clone …/openmayhem.git; Set-Location openmayhem; git checkout --detach v0.2.24; .\install.ps1 -FromSource` |

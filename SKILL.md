...
- `v0.2.41` is source-only. Never invent or offer a native archive URL: the release has no unsigned
...
| Goal | Command |
|---|---|
| Install release (macOS/Linux) | `git clone …/openmayhem.git && cd openmayhem && git checkout --detach v0.2.41 && ./install.sh --from-source` |
| Install release (PowerShell) | `git clone …/openmayhem.git; Set-Location openmayhem; git checkout --detach v0.2.41; .\install.ps1 -FromSource` |

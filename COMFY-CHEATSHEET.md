...
- Revision: `0c368cb7de7bce50223ac01c936207b523b0a774`
- Index root: `6b75023e49e1d0954ae4f76004f1cb1b620f773ed700d2cb0e55280d0a52a2a0`
- Anchor hash: `76d6b4b25ab7c1258f4fd042f4b69550b93bc586ce5869ee04a152e091ced6a4`
- Index version: `19`
- Blessed runtimes: `comfyui-v0.30.1`, `comfyui-2a68ce33b4c9`
- Parts: `109` (2 audio-model, 12 checkpoint, 1 clip-vision, 35 controlnet, 2 custom-node, 7 lipsync, 3 lora, 5 text-encoder, 20 upscaler, 9 vae, 13 video-model)
- Index URL: https://huggingface.co/datasets/TracNetwork/openmayhem-parts-index/resolve/0c368cb7de7bce50223ac01c936207b523b0a774/index.json
- Anchor URL: https://huggingface.co/datasets/TracNetwork/openmayhem-parts-index/resolve/0c368cb7de7bce50223ac01c936207b523b0a774/anchor.json
...
| Low-VRAM H3 T2V/I2V through W4A8/INT8 + turbo LoRAs | candidate `video.minimax_h3.lowvram_t2v_i2v` | Not public until the low-VRAM parts are mirrored, signed, admitted, and paid-proofed |
| Low-VRAM H3 R2V through W4A8/INT8 + REF2V turbo LoRA | candidate `video.minimax_h3.lowvram_r2v` | Not public until the low-VRAM R2V parts are mirrored, signed, admitted, and paid-proofed |
...
The submitted low-VRAM MiniMax H3 pack is a separate candidate policy family,
not a replacement for the existing H3 rows. Its T2V/I2V policy must allow only
steps `4`, `6`, and `8`; `4` is the default. The submitted R2V pack currently
contains only a 4-step workflow, so do not advertise R2V `6` or `8` until those
graphs are separately proven. Low-VRAM pack defaults are `736x1280`, `10.0s`,
`24fps`, fixed seed `4242424242`, `MiniMaxLowVRAMAttention.head_chunks=4`,
`MiniMaxChunkFeedForward.chunks=2`, and
`MiniMaxChunkFeedForward.seq_threshold=4096`. The ZIP's exported UI graphs omit
the `VAEDecode -> CreateVideo.images` link; Mayhem API graphs must include that
edge or the canary is invalid.
...
```bash
cd <comfy-runtime-dir>
RUNTIME=$PWD
VENV=<provider-home>/venvs/comfyui-v0.30.1
python3 -m venv "$VENV"
"$VENV/bin/python" -m pip install --upgrade pip wheel setuptools
"$VENV/bin/python" -m pip install -r "$RUNTIME/requirements.txt"
export MAYHEM_COMFYUI_PYTHON="$VENV/bin/python"
"$VENV/bin/python" - <<'PY'
import importlib.util
for name in ["torch", "sqlalchemy", "alembic"]:
    assert importlib.util.find_spec(name), f"missing {name}"
PY
```

Keep the selected venv outside the ComfyUI checkout. The provider sandbox mounts
the runtime checkout and interpreter environment as separate read-only roots; a
venv nested below the checkout is rejected as an overlapping sandbox root.
...
- Input-media gate: if the graph consumes image/audio/video files, the final proof must send them through `/v1/workflows` request media, not through provider-local files. The graph, provider admission, and paid route proof must reference the same safe filenames. The bytes must also fit the signed media caps before dispatch; for example, the LongCat lipsync proof had to use a `720x415` reference image because an `832x480` request image exceeded the signed `720x480` input-cap area and was correctly held at zero eligible routes before spend.
...
seven product-accepted lanes, one optional parked H3 enhancer, and one lane that
still has a bounded quality-fit gap. Optional workflow lanes are tracked
...
- `video.minimax_h3.r2v`: accepted for the REF2VA/reference-media lane after
...
- `video.minimax_h3.spectrum`: signed dev catalog row and workflow-class
  ...
  verifies with canary set `canary-minimax-h3-spectrum-workflow-launch-v1`;
  the retained reference artifact is
  `openmayhem-minimax-h3-spectrum-reference-v0.2.129.mp4`. Paid fiat route
  proof passed through the `.31` sponsored gateway on 2026-08-12 with retained
  artifact `openmayhem-minimax-h3-spectrum-paid-v0.2.136.mp4`, session
...
- `upscale.conv.le24mp`: signed standalone 4x convolutional upscaler policy
  exists using SPANx4
  `d871ba305a9cbe521c3da166f06d84b80db02a36a1b4e89720d6bddf54965e0a`.
  Catalog verification and local reference admission passed with inventory root
  `e3939d8c9af52b8772faaa0c2570e27d3cb3f963001c09b8efe42f6e48f42c98`;
  retained reference artifact:
  `openmayhem-upscale-conv-le24mp-reference-v0.2.127.png`, SHA-256
  `3b78c0ecd45cfa63c75d2eea18c7056c417015908c7ec7c8dddc327949e4f8fc`.
  It is product-accepted after paid fiat proof through the `.31` proof gateway
  against the `.42` provider on 2026-08-11; retained artifact
...
- `upscale.diffusion`: signed dev SeedVR2 diffusion upscale/restore policy is
  ...
  `e2a27b04c8c7244829fc5fbe3281cf7d29c7f65ef315fbb97386a66e2b3da7c7`
  and SeedVR2 VAE part
  ...
  A stronger real-world admission proof was also run on `.42` with the same
  signed SeedVR2 parts and graph; retained artifact:
  `openmayhem-seedvr2-realworld-upscale-admission-v0.2.138.png`, output
  SHA-256
  ...
  proof gateway against the `.70` provider; retained artifact
  ...
  BLAKE3 `9c5dd50d7abf347c528ec36914506448670a4f7087ac32c3692ac5ed0be380d4`,
  output SHA-256
  `9fe0f803f4699b882406d9271c1eafbd2f7bb415bfca7b173eb4669afd8c3d18`,
...
- `video.lipsync`: InfiniteTalk remains a technical/reference proof, not a
  ...
  proof gateway. Retained media:
  `openmayhem-longcat-paid-anime-fight-v0.2.135.mp4`, session
  `33fa30144eeb90c5086716231cffa9c6a0ad49e722b3d064b336760e2febae22`,
  BLAKE3 `acafe1209d34b02e2a97f045f507aed762a69fa71dc807521f7829ba10e9a2d3`;
  media `832x480`, `25` fps, `100` frames, `4.000s`, H.264 video plus AAC
  audio. A later paid run retained
  `openmayhem-longcat-paid-anime-fight-v0.2.138.mp4`, session
  ...
  media `768x512`, `25` fps, `120` frames, `4.800s`, H.264/AAC stereo. These
  close the paid technical route proof for talking-video lipsync only. Owner
  review found the visual proof is a static anime face-off rather than
...
| `upscale.conv.le24mp` | Standalone convolutional 4x upscale/restore up to 4096x4096 output | 4x-spanx4 `d871ba305a9cbe521c3da166f06d84b80db02a36a1b4e89720d6bddf54965e0a` | `comfyui-v0.30.1` | Product-accepted after `.42` admission and paid fiat `/v1/workflows` proof through the `.31` proof gateway; inventory root `e3939d8c9af52b8772faaa0c2570e27d3cb3f963001c09b8efe42f6e48f42c98`, required memory `4.81GiB`, graph SHA-256 `9a24896bfe8a78bcac41f7895126ad02593bde5bcabb9aec70229c7940d2f2a2`, retained reference artifact `openmayhem-upscale-conv-le24mp-reference-v0.2.127.png`, output SHA-256 `3b78c0ecd45cfa63c75d2eea18c7056c417015908c7ec7c8dddc327949e4f8fc`; paid artifact `openmayhem-upscale-conv-le24mp-paid-v0.2.128.png`, session `04a42dc2aa5159d317be3c0e80924421d1163021e91d8b41352d72ba96b940fe`, BLAKE3 `1d47264316354f7f0aa787da4637711e38a0b8e78a715682cb241a937e8f9699`. |
| `upscale.diffusion` | SeedVR2 diffusion upscale/restore | SeedVR2 3B int8 convrot `e2a27b04c8c7244829fc5fbe3281cf7d29c7f65ef315fbb97386a66e2b3da7c7`; SeedVR2 VAE `63e6908333939636708d0661208d534237a117d1a6a36f4c3544c1cff40be6a1` | `comfyui-v0.30.1` | Product-accepted after signed dev policy, `.70` reference admission, toy paid route proof, and real-world paid route proof. Inventory root `f501d4d7340fe2d891560aef0192adb28c0bd8e91c77b36e4ccc0e16b33fd15b`, required memory `14.03GiB`, graph SHA-256 `2f8f41767f90fd18a7a7109b156ef2464f9c22648ce37ada3126e02e76fc2c93`, retained reference artifact `openmayhem-seedvr2-upscale-diffusion-reference-v0.2.136.png`, reference output SHA-256 `671c24b35e99ee28edbf08b3c88c37472fbb552dfd52764f9f2d6f422149e328`; toy paid artifact `openmayhem-seedvr2-upscale-diffusion-paid-v0.2.138.png`, session `35b01e56d29b1fe11c1dd1b73fe7eca055ff35825094c7302f2a47c158bb3101`, BLAKE3 `22895752f732ddfeb17f3aaa93fbad6b3e2bf60a9e7b9c93c7bcadd3140d1dfb`, SHA-256 `e23ce117ba5325919c642a531f69cea09dd9b36a0c27fdcb41ca14790730d9ff`. Real-world paid artifact `openmayhem-seedvr2-realworld-upscale-paid-v0.2.139.png`, session `be95c4a70006f5210ae9b8d63d0d185e33067525705c33c2776d1b9a60c5883a`, BLAKE3 `9c5dd50d7abf347c528ec36914506448670a4f7087ac32c3692ac5ed0be380d4`, SHA-256 `9fe0f803f4699b882406d9271c1eafbd2f7bb415bfca7b173eb4669afd8c3d18`, media `512x288` PNG, usage `1` `megapixel_step`. |
...
| `video.minimax_h3.t2v_i2v` | MiniMax H3 text/image-to-video with native stereo audio up to 1344x768 base or 1792x1024 via optional SeedVR2 2x branch, 15s, 360 frames | Base: FL2VA diffusion `4c371bcbf8e7a577457d7b0ace66345fa85c88a591ca0724a5da6e9642371f72`; Qwen3VL 32B NVFP4 text encoder `32432239ffed7077993a928a915c0dc8252238657ecd4926335cfa8afff7e0ab`; H3 video VAE `3abef9354f37bb10b413e7034d373e95193511cd80ffa5aea315d1d822032ce7`; H3 audio VAE `6058c1f32eae8766393ece25f7e65871313c90197d76608b62b4ed5fac78dcd2`. Optional 2x branch also requires SeedVR2 3B int8 convrot `e2a27b04c8c7244829fc5fbe3281cf7d29c7f65ef315fbb97386a66e2b3da7c7` and SeedVR2 VAE `63e6908333939636708d0661208d534237a117d1a6a36f4c3544c1cff40be6a1`; branch inventory root `1165f3bb28092852c60cdc61d524bb280a45eca39017147484adf9e1d9816ec6` | `comfyui-v0.30.1` | Product-accepted for the base T2V/I2V lane: parts mirrored in parts-index v12, signed catalog metadata published, enclave/price/room live on mainnet, `.70` admission passed, and retained paid `/v1/workflows` proofs passed quality review. Fresh live proof: `.70` served through the `.31` sponsored fiat gateway on 2026-08-11; retained artifact `openmayhem-minimax-h3-t2v-paid-proof-v0.2.127.mp4`, session `02e4b50f367548e156d1ca47975c1e2d213138bf4c1110311f87417af9173e58`, BLAKE3 `f258297d464d7ee060ed9f49f38008b3880379c3cf29adb922769c8c577b7ffc`. Stronger dialogue-oriented artifact: `openmayhem-minimax-h3-anime-dialogue-fight-paid-v0.2.123.mp4`, session `286e0bf679ae7dc44c34f4979a4b53affd6c209a8ecfe774eef9380dffada1b8`, BLAKE3 `431caae7624d36a15917a1f7ee60fdd47118b35c5e025f2efc4c41aca372aae1`. Optional SeedVR2 2x branch proof retained in Downloads: base `896x512`/124 frames generated in `340.669s`, upscaled `1792x1024`/124 frames in `440.957s`, total `781.626s`; H3 T2V/I2V billing is `$0.694444444444444444/1000` `megapixel_step`, making a 15s 896x512 base run about `$0.25` and the 1792x1024 2x branch about `$0.50`. |
| `video.minimax_h3.r2v` | MiniMax H3 reference-to-video / reference-media workflow | REF2VA diffusion `b5f18df20fb79f5ae577ed27d16182251712d9a1f30a29af3ffbd6526356b87b`; Qwen3VL 32B NVFP4 text encoder `32432239ffed7077993a928a915c0dc8252238657ecd4926335cfa8afff7e0ab`; H3 video VAE `3abef9354f37bb10b413e7034d373e95193511cd80ffa5aea315d1d822032ce7`; H3 audio VAE `6058c1f32eae8766393ece25f7e65871313c90197d76608b62b4ed5fac78dcd2` | `comfyui-v0.30.1` | Product-accepted after signed v13 parts, `.70` admission, live route, and a paid fiat `/v1/workflows` proof through the `.31` sponsored gateway; retained artifact `openmayhem-minimax-h3-r2v-paid-proof-v0.2.126.mp4`, session `a938e5efd7ddba9a610dbf16723d7a6da62c6e6f8c9dfe924f4765c0111ba81c`, BLAKE3 `19f92129c62ef352a2460a1a3d8654d957672fa1ae4cf0c138b3fdaea468cbfd`. The request-carried two-audio-reference route is covered by `openmayhem-minimax-h3-r2v-audio-reference-anime-fight-v0.2.139.mp4`, BLAKE3 `1b622c70387b73ac667dd77440699eadb882cf7090ff3f3dfd6167c227ce130f`. |
| candidate `video.minimax_h3.lowvram_t2v_i2v` | MiniMax H3 low-VRAM text/image-to-video with native audio, 736x1280, 10s, 243 frames | W4A8 FL2VA `minimax_h3_fl2va_pruned_w4a8_mixed.safetensors`; Qwen3VL NVFP4 text encoder `32432239ffed7077993a928a915c0dc8252238657ecd4926335cfa8afff7e0ab`; INT8 video VAE `minimax_h3_video_vae_int8_convrot.safetensors`; H3 audio VAE `6058c1f32eae8766393ece25f7e65871313c90197d76608b62b4ed5fac78dcd2`; turbo LoRAs for `4`, `6`, and `8` steps; custom-node payloads `ComfyUI-MiniMaxH3-Easy@8138215` and `ComfyUI-KJNodes@6ab7e81` | Candidate runtime: first try `comfyui-v0.30.1`; split to a newer blessed runtime only if render proof fails | Candidate only. The workflow policy must set `allowed_steps: [4, 6, 8]`, `max_steps: 8`, default `steps: 4`, and reject 20-step graphs. Missing parts are pinned in `docs/comfy/ImageVideoGenModelsListOpenmayhem.yaml` but are not yet mirrored/signed in the active parts index. No public provider may advertise this until signed parts, admission, canary, and paid proof pass. |
| candidate `video.minimax_h3.lowvram_r2v` | MiniMax H3 low-VRAM reference-to-video with native audio, 736x1280, 10s, 243 frames | W4A8 REF2VA `minimax_h3_ref2va_pruned_w4a8_mixed.safetensors`; shared text/audio parts; INT8 video VAE; 4-step REF2V turbo LoRA; same low-VRAM custom-node payloads | Candidate runtime: first try `comfyui-v0.30.1`; split only if proof fails | Candidate only. The submitted pack has only a 4-step R2V graph, so the R2V policy must default to 4 and not advertise 6/8 until separately proven. |
| `video.minimax_h3.spectrum` | Optional MiniMax H3 Spectrum enhancer workflow | FL2VA diffusion `4c371bcbf8e7a577457d7b0ace66345fa85c88a591ca0724a5da6e9642371f72`; Qwen3VL 32B NVFP4 text encoder `32432239ffed7077993a928a915c0dc8252238657ecd4926335cfa8afff7e0ab`; H3 video VAE `3abef9354f37bb10b413e7034d373e95193511cd80ffa5aea315d1d822032ce7`; H3 audio VAE `6058c1f32eae8766393ece25f7e65871313c90197d76608b62b4ed5fac78dcd2`; Spectrum custom node `d9a5deca829e775cf5750c203d00499d5528bcc276fb9a84a4722ca133a9f52f` | `comfyui-v0.30.1` | Signed dev catalog row, workflow-class canary proof, and paid fiat proof passed; catalog hash `debf0574baf90c286e132a518986c99ddf2789890cd7b8edb7262c54694fc34a`, canary set `canary-minimax-h3-spectrum-workflow-launch-v1`, retained reference artifact `openmayhem-minimax-h3-spectrum-reference-v0.2.129.mp4`, graph SHA-256 `6a2ffd3580201483c1fa34deedb79e022235d817b7185e8615159b61d874bf71`. Paid artifact `openmayhem-minimax-h3-spectrum-paid-v0.2.136.mp4`, session `c51f1fe861e46a6ebe680f1c57670f2982f81a782db7a9ce93c384ad78af7e6f`, BLAKE3 `25b0a156e76ad22653cee1037f6bc0fb64955e668bc5b3ecc3af39a4915ed8f2`, media `896x512`, `24` fps, `5.167s`, H.264/AAC stereo. Follow-up comparison against base H3 found no measurable product lift; parked as optional, not product-accepted. |
| `video.lipsync` | LongCat Avatar 1.5 lipsync/talking-video workflow up to 832x480, 4s, 100 frames | LongCat INT8 shards `03aec7302e68dd520503e1d86b3bd737676081c6d7bc87e23b958907c0e0d763`, `96024250fff268d22b3fe795de40cb12f326b9c0b78a64677521e6458b4cddbd`, `28e2da04259e4f557f906b16bc0ee9f9eb8abbfbdbe01991d217b079c622685a`, `1d5b361c161b284ef03be517230e82b88dff29c20ea4c40da153af4084655272`; index/config `94477286a0e2869d55f4e5fbe1d724957064ec8489c247fcefaf38004428a476`, `de863798826508a9aeedb68b74d1389b0dad8161098b4f150fb36c90353fc1c2`, `4c9359ce6feee6b6d2bedac09e5e0b26519efc1a35d1ddda1737518669bb03dd`; DMD LoRA `421eaeb552cec6e21c506bc1583b1a05ace4d82501a463611d1db033427f86ec`; VAE `6b437f1eb894131d58bebdfb6e8b7397e7031d945e8aa0e8712791344f11b4e5`; UMT5 fp8 `720ea5ea7b9de57ca87b403856b0a7e42c96d1f1176ff886726ab602b6923709`; Whisper-large-v3 `009d64bc53708635cca4b6cf2b4603bdf474e989c70ed47d550cca90b48f8c95`; custom node `29b4d476aab99bc848b06718d4b857f79fb155ab6ceac12ddf8f8b69c2cf99e3` | `comfyui-longcat-avatar-v0.30.1` | Reference-admitted on `.42` using signed inventory root `d301dcad94837f8b29471de0e7b78d0108dd08d26d273df834dfbcb3ab9ca88b`; required memory `32.12GiB`; graph SHA-256 `7f6df53d5659f072ee4205765d6c78c7f7dc172273ca1d8ff0b19c1ad2f0431c`; retained reference artifact `openmayhem-longcat-avatar-anime-fight-dialogue-reference-v0.2.129.mp4`, output SHA-256 `d6e2622e2474c82f5324eecf4359c7877e14edbe77f811081da305460c4fc872`. Paid fiat proofs passed through the `.31` proof gateway using request-carried image/audio media under the signed caps; retained artifacts `openmayhem-longcat-paid-anime-fight-v0.2.135.mp4` and `openmayhem-longcat-paid-anime-fight-v0.2.138.mp4`. Quality review: technically valid talking-video/lipsync route, but static face-off rather than action choreography; not public product-accepted for the general anime-action-with-dialogue target. |

### Optional Calibration Targets Not Product-Accepted

| Target policy | Purpose | Parts needed | Missing work before serving |
|---|---|---|---|
| `video.minimax_h3.spectrum` | Optional H3 Spectrum enhancer lane | H3 base parts plus Spectrum H3 extension payload `d9a5deca829e775cf5750c203d00499d5528bcc276fb9a84a4722ca133a9f52f` | Signed dev catalog row, workflow-class canary proof, and paid fiat `/v1/workflows` route proof exist. Follow-up comparison against base H3 found no measurable product lift, so this lane is parked and not product-accepted. |
| candidate `video.minimax_h3.lowvram_t2v_i2v` | 16GB-class H3 T2V/I2V lane from the submitted low-VRAM pack | W4A8 FL2VA, INT8 video VAE, shared text/audio parts, 4/6/8-step turbo LoRAs, MiniMaxH3-Easy nodes, KJ low-VRAM nodes | Mirror/sign missing parts, publish a separate policy with `allowed_steps: [4, 6, 8]`, prove current runtime or split runtime, then run admission and paid proof. |
| candidate `video.minimax_h3.lowvram_r2v` | 16GB-class H3 R2V lane from the submitted low-VRAM pack | W4A8 REF2VA, shared text/audio/video parts, 4-step REF2V LoRA, same custom nodes | Mirror/sign missing parts, publish a separate 4-step policy, prove current runtime or split runtime, then run admission and paid proof. |
...
same final proof path has passed.
...
mainnet catalog points to revision `6e728bb10404a9d33283625f3d67846db1f9a919`,
and the ledger has the base and R2V H3 enclaves, prices, and canonical rooms
open. H3
...
The R2V lane completed paid fiat proof through the `.31` sponsored gateway in
v0.2.126 with retained artifact
`openmayhem-minimax-h3-r2v-paid-proof-v0.2.126.mp4`, `896x512`, `124` frames at
...
The signed R2V canary exercises reference images only. Official Comfy H3 R2V
templates also expose standalone audio references through `ref_audios` and
prompt tags such as `<Audio 1>`. The accepted R2V policy now permits bounded
request-carried audio references: audio `max_items_per_request` and
`max_inflight_items` are `2`, enough for a two-speaker reference proof without
opening unbounded media fan-in. Required proof remains request-carried audio in
...
- `nkxx188/ComfyUI-MiniMaxH3-Easy` at commit
  ...
  prompt optimization.
- The submitted low-VRAM H3 workflow pack pins
  `nkxx188/ComfyUI-MiniMaxH3-Easy` at
  `8138215b08912f8fc30410c3a01b2574f6c46dd7` and
  `kijai/ComfyUI-KJNodes` at
  `6ab7e8130e449ed2c0037589bcf84146ceb7fc9c`. It targets
  `736x1280`, `10.0s`, `24fps`, W4A8 diffusion, INT8 video VAE,
  `head_chunks=4`, and feed-forward chunks `2`. Treat this as a separate
  low-VRAM policy proposal with T2V/I2V allowed steps `4`, `6`, and `8`
  and default `4`; do not merge it into the existing 20-step H3 class.
- `xmarre/ComfyUI-Spectrum-MiniMax-H3` `v0.2.1` is an owner-approved
  optional enhancer/acceleration calibration target for H3. Its release makes
...
| SeedVR2 diffusion upscale | video-model | seedvr2_3b_int8_convrot.safetensors | `e2a27b04...da7c7` | 3.22 GiB | apache-2.0 | [record](https://huggingface.co/datasets/TracNetwork/openmayhem-parts-index/blob/0c368cb7de7bce50223ac01c936207b523b0a774/records/e2a27b04c8c7244829fc5fbe3281cf7d29c7f65ef315fbb97386a66e2b3da7c7.json) |

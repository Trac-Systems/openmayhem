...
| Low-VRAM H3 T2V/I2V through W4A8/INT8 + turbo LoRAs | `video.minimax_h3.lowvram_t2v_i2v` | A live low-VRAM T2V/I2V provider on the matching release/catalog |
| Low-VRAM H3 R2V through W4A8/INT8 + REF2V turbo LoRA | `video.minimax_h3.lowvram_r2v` | A live low-VRAM R2V provider on the matching release/catalog |
...
The submitted low-VRAM MiniMax H3 pack is a separate policy family,
...
| `video.minimax_h3.lowvram_t2v_i2v` | MiniMax H3 low-VRAM text/image-to-video with native audio, 736x1280, 10s, 243 frames | W4A8 FL2VA `minimax_h3_fl2va_pruned_w4a8_mixed.safetensors`; Qwen3VL NVFP4 text encoder `32432239ffed7077993a928a915c0dc8252238657ecd4926335cfa8afff7e0ab`; INT8 video VAE `minimax_h3_video_vae_int8_convrot.safetensors`; H3 audio VAE `6058c1f32eae8766393ece25f7e65871313c90197d76608b62b4ed5fac78dcd2`; turbo LoRAs for `4`, `6`, and `8` steps; custom-node payloads `ComfyUI-MiniMaxH3-Easy@8138215` and `ComfyUI-KJNodes@6ab7e81` | `comfyui-v0.32.0` | Source-ready after signed parts, converted policy, and Windows CUDA reference admission proofs for T2V 4-step, T2V 6-step, and I2V 8-step. The workflow policy sets `allowed_steps: [4, 6, 8]`, `max_steps: 8`, default `steps: 4`, and rejects 20-step graphs. It is not public-live until the matching release/catalog, provider admission, and paid route proof exist. |
| `video.minimax_h3.lowvram_r2v` | MiniMax H3 low-VRAM reference-to-video with native audio, 736x1280, 10s, 243 frames | W4A8 REF2VA `minimax_h3_ref2va_pruned_w4a8_mixed.safetensors`; shared text/audio parts; INT8 video VAE; 4-step REF2V turbo LoRA; same low-VRAM custom-node payloads | `comfyui-v0.32.0` | Source-ready after signed parts, converted policy, and Windows CUDA reference admission proof for R2V 4-step. The submitted pack has only a 4-step R2V graph, so the R2V policy defaults to 4 and must not advertise 6/8 until separately proven. It is not public-live until the matching release/catalog, provider admission, and paid route proof exist. |
...
| `video.minimax_h3.lowvram_t2v_i2v` | 16GB-class H3 T2V/I2V lane from the submitted low-VRAM pack | W4A8 FL2VA, INT8 video VAE, shared text/audio parts, 4/6/8-step turbo LoRAs, MiniMaxH3-Easy nodes, KJ low-VRAM nodes | Source-ready after `comfyui-v0.32.0` reference proofs; not public-live until release/catalog publication and paid route proof. |
| `video.minimax_h3.lowvram_r2v` | 16GB-class H3 R2V lane from the submitted low-VRAM pack | W4A8 REF2VA, shared text/audio/video parts, 4-step REF2V LoRA, same custom nodes | Source-ready after `comfyui-v0.32.0` R2V reference proof; not public-live until release/catalog publication and paid route proof. |
...
- The submitted low-VRAM H3 workflow pack pins
  ...
  low-VRAM policy family on `comfyui-v0.32.0` with T2V/I2V allowed steps
  `4`, `6`, and `8` and default `4`; do not merge it into the existing
  20-step H3 class. The R2V lane is separate and 4-step only.

...
import Contract23 from '../contract/history/v23.js';import Contract24 from '../contract/history/v24.js';import Contract25 from '../contract/history/v25.js';import Contract26 from '../contract/history/v26.js';import Contract27 from '../contract/history/v27.js';
...
async function fixture(version,kind='feature'){
 ...
 const Implementation=version===23?Contract23:version===24?Contract24:version===25?Contract25:version===26?Contract26:Contract27;const old=new Implementation(proto,{});let store,base,lastNode;
...
for(const version of [23,24,25,26,27])for(const kind of ['feature','tx'])test(`persisted signed v${version} ${kind} history preserves full tree under28 exactly once`,async()=>{
...
test('fresh, unbound, mutated and wrong-version historical Feature calls remain rejected',async()=>{
 ...
 await assert.rejects(c.execute(f.op,f.before()),/expected CONTRACT_VERSION 28, got 24/);
 await assert.rejects(c.execute(f.op,f.before(),{replay:true}),/expected CONTRACT_VERSION 28, got 24/);
 ...
 const unsignedNode={...f.node,value:fresh};await assert.rejects(handler(f.wallet,c,f.view).handle(fresh,f.before(),{},unsignedNode),/expected CONTRACT_VERSION 28, got 24/);
 const bad=structuredClone(f.op);bad.value.dispatch.contract_version=23;await assert.rejects(handler(f.wallet,c,f.view).handle(bad,f.before(),{},{...f.node,value:bad}),/expected CONTRACT_VERSION 28, got 23/);
...
test('ordinary historical TX requires exact canonical index and original signed bytes',async()=>{
 const f=await fixture(24,'tx');try{const c=new MayhemContract(f.proto,{}),storage=f.before();await assert.rejects(c.execute(f.op,storage),/expected CONTRACT_VERSION 28, got 24/);
...
for (const version of [23, 24, 25, 26, 27]) test(`persisted remote v${version} Feature reads signed default view ahead of apply batch`, async () => {
 ...
 const old=new (version===23?Contract23:version===24?Contract24:version===25?Contract25:version===26?Contract26:Contract27)(protocol,{});
 ...
 try{
  ...
  await until(()=>broken.error);assert.match(broken.error.message,new RegExp('expected CONTRACT_VERSION 28, got '+version));

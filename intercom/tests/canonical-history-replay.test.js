...
async function fixture(version,kind='feature'){
 ...
 const Implementation=version===23?Contract23:Contract24;const old=new Implementation(proto,{});let store,base,lastNode;
 ...
 if(kind==='feature'){
  ...
  const paid=await proto.preparePaidTransaction({type:'setParams',value:{op:'set_params',submitted_at:0,effective_at:86400,values:{price_min_bps:1}}});proto.versionedTransactionObject=original;
...
for(const version of [23,24])for(const kind of ['feature','tx'])test(`persisted signed v${version} ${kind} history preserves full tree under25 exactly once`,async()=>{
 const f=await fixture(version,kind);try{const replayed=await f.replay();assert.equal(replayed.snapshotBytes(),f.expected.snapshotBytes());if(kind==='tx')assert.equal((await replayed.get('params/price_min_bps')).value.pending.value,1,'Historical pre25 pricing bound must not be reinterpreted');}finally{await f.close();}
});
test('fresh, unbound, mutated and wrong-version historical Feature calls remain rejected',async()=>{
 ...
 await assert.rejects(c.execute(f.op,f.before()),/expected CONTRACT_VERSION 25, got 24/);
 await assert.rejects(c.execute(f.op,f.before(),{replay:true}),/expected CONTRACT_VERSION 25, got 24/);
 ...
 const unsignedNode={...f.node,value:fresh};await assert.rejects(handler(f.wallet,c,f.view).handle(fresh,f.before(),{},unsignedNode),/expected CONTRACT_VERSION 25, got 24/);
 const bad=structuredClone(f.op);bad.value.dispatch.contract_version=23;await assert.rejects(handler(f.wallet,c,f.view).handle(bad,f.before(),{},{...f.node,value:bad}),/expected CONTRACT_VERSION 25, got 23/);
...
test('ordinary historical TX requires exact canonical index and original signed bytes',async()=>{
 const f=await fixture(24,'tx');try{const c=new MayhemContract(f.proto,{}),storage=f.before();await assert.rejects(c.execute(f.op,storage),/expected CONTRACT_VERSION 25, got 24/);

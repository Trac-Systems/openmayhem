...
mod tests {
    ...
    fn intercom_manifest_for(files: &[(&str, &[u8])]) -> IntercomBundleManifest {
        intercom_manifest_for_version(files, "0.2.25")
    }
    ...
    fn stage_release() -> (TestDir, ReleaseBundleManifest, Vec<u8>) {
        stage_release_for("v0.2.25", "aarch64-apple-darwin")
    }
    ...
    fn staged_source_git_sha_is_reported_and_bound_by_hash_and_signature() {
        ...
        initialize_release_anti_rollback_floor(&floor_path, "0.2.25")
    ...
    fn tar_release_is_bounded_staged_and_reauthenticated_from_live_trust() {
        ...
        initialize_release_anti_rollback_floor(&higher_floor_path, "0.2.26")
    ...
    fn windows_zip_release_is_safely_staged_and_reauthenticated() {
        let (release, manifest, manifest_bytes) =
            stage_release_for("v0.2.25", "x86_64-pc-windows-msvc");
    ...
    fn release_version_binding_accepts_optional_v_and_rejects_drift() {
        ...
        verify_release_version_binding("0.2.25", &intercom).expect("bind plain version");
        verify_release_version_binding("v0.2.25", &intercom).expect("bind tagged version");
        assert!(verify_release_version_binding("v0.2.26", &intercom)
            .expect_err("version drift must fail")
            .to_string()
            .contains("does not bind"));
        assert!(verify_release_version_binding("release-0.2.25", &intercom)
    ...
    fn activation_journal_phase_transitions_are_append_only() {
        ...
        let journal = ActivationJournal {
            ...
            release_version: "0.2.25".to_owned(),
    ...
    fn create_crashed_release_activation(
        ...
        write_release_anti_rollback_floor(&incoming_floor, "0.2.25")

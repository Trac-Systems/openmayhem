...
pub(super) async fn check_github_update(
    ...
    let checks_complete = release.is_ok() && compare.is_ok();
...
fn evaluate_github_update(
    ...
    let source_update = compare.and_then(|compare| {
        if !matches!(compare.status.as_str(), "ahead" | "diverged") || compare.ahead_by == 0 {
            return None;
        }
        Some(GatewayGithubUpdate {
    ...
    if update.is_none() && !checks_complete {
...
mod tests {
    ...
    #[test]
    fn incomplete_checks_never_claim_the_build_is_current() {
        let status = evaluate_github_update(
            "0.2.0",
            Some("1111111111111111111111111111111111111111"),
            None,
    ...
    #[test]
    fn a_proven_update_remains_visible_when_the_other_check_fails() {
        let status = evaluate_github_update(
            "0.2.0",
            Some("1111111111111111111111111111111111111111"),
            None,
        ...
        assert_eq!(status.update.expect("source update").kind, "source");
    }

    #[test]
    fn diverged_source_with_new_main_commits_is_an_update() {
        let mut compare = compare(2);
        compare.status = "diverged".to_owned();
        let status = evaluate_github_update(
            "0.2.0",
            Some("1111111111111111111111111111111111111111"),
            None,

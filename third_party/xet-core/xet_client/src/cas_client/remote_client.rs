...
use http::header::{CONTENT_LENGTH, CONTENT_RANGE, HeaderMap, RANGE};
...
const MAX_SINGLE_RANGE_RESUMES: usize = 32;

fn validate_partial_content_range(response: &Response, requested: HttpRange) -> Result<()> {
    if response.status() != StatusCode::PARTIAL_CONTENT {
        return Err(ClientError::Other(format!(
            "storage range request {requested} returned HTTP {}; expected 206",
            response.status()
        )));
    }
    ...
    let header = response
        .headers()
        .get(CONTENT_RANGE)
        .ok_or_else(|| ClientError::Other("storage range response omitted Content-Range".to_string()))?
        .to_str()
        .map_err(|_| ClientError::Other("storage returned a non-text Content-Range header".to_string()))?;
    let actual = header
        .strip_prefix("bytes ")
        .and_then(|value| value.split('/').next())
        .ok_or_else(|| ClientError::Other(format!("storage returned malformed Content-Range: {header}")))?;
    let actual = HttpRange::try_from(actual)
        .map_err(|_| ClientError::Other(format!("storage returned malformed Content-Range: {header}")))?;
    if actual != requested {
        return Err(ClientError::Other(format!(
            "storage returned Content-Range {actual}; requested {requested}"
        )));
    }
    Ok(())
}

async fn request_xorb_range(
    ctx: XetContext,
    http_client: Arc<ClientWithMiddleware>,
    url_info: Arc<Box<dyn URLProvider>>,
    range: HttpRange,
) -> Result<Response> {
    let api_tag = "s3::get_range_resume";
        ...
        .run(move || {
            ...
            async move {
                ...
                let response = http_client
                    .get(url)
                    .header(RANGE, range.range_header())
...
async fn read_single_range_with_resume(
    mut response: Response,
    requested: HttpRange,
    ctx: XetContext,
    http_client: Arc<ClientWithMiddleware>,
    url_info: Arc<Box<dyn URLProvider>>,
    transfer_reporter: StreamProgressReporter,
) -> Result<Bytes> {
    let expected = requested.length();
    let capacity = usize::try_from(expected).unwrap_or(0);
    let mut compressed = Vec::with_capacity(capacity);
    let mut resumes = 0usize;

    loop {
        let suffix_start = requested
            .start
            .checked_add(compressed.len() as u64)
            .ok_or_else(|| ClientError::Other("storage range resume offset overflowed".to_string()))?;
        let suffix = HttpRange::new(suffix_start, requested.end);
        validate_partial_content_range(&response, suffix)?;
        ...
        loop {
            match stream.try_next().await {
                Ok(Some(chunk)) => {
                    ...
                    transfer_reporter.report_progress(compressed.len());
                    if compressed.len() as u64 > expected {
                        return Err(ClientError::Other(format!(
                            "storage range {requested} exceeded its expected {expected} bytes"
                        )));
                    }
                },
                Ok(None) => break,
                Err(error) => {
                    ...
                    info!(
                        range = %requested,
                        received = compressed.len(),
                        expected,
                        error_kind,
                        "Storage range response ended early; resuming its missing suffix"
                    );
                    break;
                },
            }
        ...
        if compressed.len() as u64 == expected {
            return Ok(Bytes::from(compressed));
        }
        if compressed.len() == before {
            return Err(ClientError::Other(format!(
                "storage range {requested} made no progress at byte {}",
                compressed.len()
            )));
        }
        if resumes >= MAX_SINGLE_RANGE_RESUMES {
            return Err(ClientError::Other(format!(
                "storage range {requested} remained incomplete after {MAX_SINGLE_RANGE_RESUMES} suffix resumes"
            )));
        }

        resumes += 1;
        let next_start = requested.start + compressed.len() as u64;
        response = request_xorb_range(
            ctx.clone(),
            http_client.clone(),
            url_info.clone(),
            HttpRange::new(next_start, requested.end),
        )
        .await?;
    }
}
...
impl Client for RemoteClient {
    ...
    async fn get_file_term_data(
        ...
        let single_range = (url_ranges.len() == 1).then(|| url_ranges[0]);
        ...
        let request_url_info = url_info.clone();
        let resume_url_info = url_info.clone();
        let resume_http_client = http_client.clone();
        let resume_ctx = self.ctx.clone();
        ...
        let result = RetryWrapper::new(self.ctx.clone(), api_tag)
            ...
            .run_and_extract_custom(
                move || {
                    let http_client = http_client.clone();
                    let url_info = request_url_info.clone();
                ...
                move |resp: Response| {
                    ...
                    let resume_url_info = resume_url_info.clone();
                    let resume_http_client = resume_http_client.clone();
                    let resume_ctx = resume_ctx.clone();
                    async move {
                        ...
                        if is_multipart {
                        ...
                        } else if let Some(single_range) = single_range {
                            let compressed = read_single_range_with_resume(
                                resp,
                                single_range,
                                resume_ctx,
                                resume_http_client,
                            ...
                            .await
                            .map_err(RetryableReqwestError::RetryableError)?;
                            let (data, chunk_indices) = xet_core_structures::xorb_object::deserialize_chunks(
                                &mut std::io::Cursor::new(compressed.as_ref()),
                            )
                            .map_err(|e| RetryableReqwestError::RetryableError(ClientError::FormatError(e)))?;
                            if let Some(expected) = uncompressed_size_if_known
                                && expected != data.len()
                            {
                                return Err(RetryableReqwestError::RetryableError(ClientError::Other(format!(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_single_range_resumes_after_premature_eof() {
        let server = MockServer::start().await;
        let payload = b"a complete xorb chunk split across two HTTP responses";
        let mut serialized = Vec::new();
        serialize_chunk(payload, &mut serialized, CompressionScheme::None).unwrap();

        let split = serialized.len() / 2;
        let end = serialized.len() - 1;
        let full_header = format!("bytes=0-{end}");
        let suffix_header = format!("bytes={split}-{end}");

        Mock::given(method("GET"))
            .and(path("/xorb"))
            .and(header("range", full_header.as_str()))
            .respond_with(
                ResponseTemplate::new(206)
                    .insert_header("content-range", format!("bytes 0-{end}/{}", serialized.len()))
                    .set_body_bytes(serialized[..split].to_vec()),
            )
            .expect(1)
            .mount(&server)
            .await;
        Mock::given(method("GET"))
            .and(path("/xorb"))
            .and(header("range", suffix_header.as_str()))
            .respond_with(
                ResponseTemplate::new(206)
                    .insert_header(
                        "content-range",
                        format!("bytes {split}-{end}/{}", serialized.len()),
                    )
                    .set_body_bytes(serialized[split..].to_vec()),
            )
            .expect(1)
        ...
        let config = XetConfig::new()
            .with_config("client.retry_max_attempts", "1")
            .unwrap()
            .with_config("client.retry_base_delay", "1ms")
            .unwrap();
        let ctx = XetContext::from_external(tokio::runtime::Handle::current(), config);
        let client = RemoteClient::new(ctx, "http://unused", &None, "test", false, None);
        let provider: Box<dyn URLProvider> = Box::new(StaticURLProvider {
            url: format!("{}/xorb", server.uri()),
            range: HttpRange::new(0, end as u64),
        });

        let permit = client.acquire_download_permit().await.unwrap();
        let (downloaded, _) = client
            .get_file_term_data(provider, permit, None, Some(payload.len()))
            .await
            .unwrap();

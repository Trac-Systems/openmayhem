...
impl RetryWrapper {
    ...
    pub fn with_retry_on_401(mut self) -> Self {
        self.retry_on_401 = true;
        self
    }
...
impl std::fmt::Debug for RetryWrapper {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RetryWrapper")
            ...
            .field("retry_on_401", &self.retry_on_401)
    ...
    fn process_ok_response(
        ...
        match (resp.error_for_status(), retriability) {
            (Err(e), Some(Retryable::Fatal)) => {
                if e.status() == Some(StatusCode::UNAUTHORIZED) && self.retry_on_401 {
                    let cas_err = process_error("Retry on 401 (Unauthorized) enabled", e, true);
                    Err(RetryableReqwestError::RetryableError(cas_err))
                // Intercept the forbidden condition if retry on 403 is enabled.
                } else if e.status() == Some(StatusCode::FORBIDDEN) && self.retry_on_403 {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_401_no_retry_by_default() {
        let server = MockServer::start().await;
        ...
        let result = connection_wrapper("test_401_no_retry_by_default")
            .with_max_attempts(3)
            .run(move || {
                let url = format!("{}/unauthorized", server.uri());
                counter_.fetch_add(1, Ordering::Relaxed);
                client.clone().get(&url).send()
            })
            .await;
    ...
    #[tokio::test]
    async fn test_401_retry_then_success_when_enabled() {
        let server = MockServer::start().await;

        Mock::given(method("GET"))
            .and(path("/unauthorized_then_ok"))
            .respond_with(ResponseTemplate::new(401))
            .up_to_n_times(2)
            .mount(&server)
            .await;
        ...
        let result = connection_wrapper("test_401_retry_then_success_when_enabled")
            .with_max_attempts(3)
            .with_retry_on_401()
            .run(move || {
                let url = format!("{}/unauthorized_then_ok", server.uri());
                counter_.fetch_add(1, Ordering::Relaxed);
                client.clone().get(&url).send()
            })
            .await;

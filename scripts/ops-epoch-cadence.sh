...
# An epoch with retained receipts is NEVER sealed away; it must be settled with
# the documented manual pipeline (receipts export -> recompute-epoch-roots ->
# admin epoch-commit -> admin epoch-apply) so providers get paid. This script
# then advances past it on the next run.
set -euo pipefail
...
if [[ -z "$updated_epoch" ]]; then
    log "abort: could not read epoch/apply/state.updated_epoch"
    exit 1
fi
...
if [[ -z "$sessions_active" || "$sessions_active" != "0" ]]; then
...
if [[ "$retained_receipts" != "0" ]]; then
    log "ATTENTION: gateway retains $retained_receipts receipt(s); manual settlement required (receipts export -> recompute -> epoch-commit -> epoch-apply). Not sealing epoch $target."
    exit 0
fi

...
write_intercom_metadata_fixture() {
  ...
  printf '%s\n' \
    '{"schema":1,"release_version":"0.2.25","contract_version":1,"contract_code_sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","assets":[]}' \
    >"$output"
}
...
assert_layout() {
  ...
  local archive_basename="mayhem-0.2.25-$target"
...
if (genericManifest.schema !== 1 ||
    ...
    genericManifest.version !== '0.2.25' ||
...
test_target() {
  ...
  [[ "$executable_name" == \
    "mayhem-attestation-verifier-0.2.25-$target$extension" ]] ||
    fail "managed verifier executable name is not canonical for $target"
  [[ "$managed_manifest_name" == \
    "mayhem-attestation-verifier-0.2.25-$target.manifest.json" ]] ||

...
// operator - burned), 15% OPERATOR CAP, residual burn allocation, claimed[acct] never exceeds rooted cumulative. Attacks:
...
try {
  ...
  const assertInvariants = async (where) => {
    ...
    const providerCap = (spent * 7500n) / 10000n;
    const burnCap = spent - providerCap - opCap;
  ...
  for (let e = 1; e <= EPOCHS; e++) {
    ...
    const governanceSignature = await signRootProposal({
      ...
      pool,
      merkleRoot: lastDist.root,
      newEpoch: e,
      newCumulativeSpent: spent,
    });
    await (await pool.connect(operator).proposeRoot(lastDist.root, e, spent, governanceSignature)).wait();
    await (await pool.executeRoot()).wait();
    lastEpoch = e;
    await assertInvariants(`epoch ${e} governed root`);

    // each provider claims its current cumulative ~70% of the time
    for (let i = 0; i < 3; i++) {
      if (cum[i] > claimedLocal[i] && r() < 0.7) {
        await (await pool.connect(provs[i]).claim(e, provAddr[i], cum[i], lastDist.proofFor(provAddr[i]))).wait();
    ...
    const burnable = spent - ((spent * 7500n) / 10000n) - ((spent * 1500n) / 10000n) - burned;
  ...
  if (claimedLocal[i0] === cum[i0] && cum[i0] > 0n) {
    await expectRevert(() => pool.connect(provs[i0]).claim.staticCall(lastEpoch, provAddr[i0], cum[i0], lastDist.proofFor(provAddr[i0])), 'double-claim same cumulative reverts');
  }
  // claim-beyond-owed: forge a higher amount with the real proof -> bad proof
  await expectRevert(() => pool.connect(provs[i0]).claim.staticCall(lastEpoch, provAddr[i0], cum[i0] + 1n, lastDist.proofFor(provAddr[i0])), 'claim-beyond-owed (forged amount) reverts');
  // wrong-account proof: claim prov0's amount with prov1's proof
  await expectRevert(() => pool.connect(provs[i0]).claim.staticCall(lastEpoch, provAddr[i0], cum[i0], lastDist.proofFor(provAddr[1])), 'mismatched proof reverts');
  ...
  // non-owner root proposal
  await expectRevert(() => pool.connect(provs[0]).proposeRoot.staticCall(lastDist.root, EPOCHS + 1, spent, '0x'), 'non-owner proposeRoot reverts');
  // burn the remaining accrued 10%, then a re-burn with nothing accrued must revert (cap exhausted)
  if ((await pool.burnClaimable()) > 0n) {
    await (await pool.connect(provs[0]).burn()).wait();
    burned = spent - ((spent * 7500n) / 10000n) - ((spent * 1500n) / 10000n);
  }
  await expectRevert(() => pool.connect(provs[0]).burn.staticCall(), 'burn with nothing accrued reverts');
  ok((await token.balanceOf(DEAD)) === burned, 'dead-address holds exactly the cumulative burn plus split remainder');
